            ...
            steps {
                withCredentials([
                    ...
                    script {
                        ...
                        // Jenkins-safe parsing (NO /regex/ literal, NO jq)
                        def matcher = resp =~ '"secret"\\s*:\\s*"([^"]+)"'
                        if (!matcher.find()) {
                            error "Databricks secret not found in API response"
                        }

                        env.NEW_VALUE = matcher.group(1)
                    }
                }
            }
        }
